...
**Update — removed for local dev (2026-09-07):** the section below documents why Docker was adopted; it's kept for the reasoning, not as current instructions. In practice, `docker-compose.dev.yml`'s bind-mounted `web` container hit a recurring `next dev` crash-loop on macOS (Docker Desktop's virtiofs/gRPC-FUSE file-watching under the bind mount, the "honest caveat" below made good on) — the process would exit and silently relaunch, so Fast Refresh sometimes required a manual restart to actually show a change. Day-to-day dev is now native-only (`uv run` + `pnpm dev`, per the README), which was already the lower-friction option per the "Day-to-day development" note below and had none of this flakiness. `docker/`, both `Dockerfile`s per side, and the compose files are deleted. The Phase 4 tool-runner sandboxing gap this section describes is still real and still unsolved — `FileEditorExecutor`'s allow-list (`backend/tapestry/tools/file_editor.py`) remains the only boundary — it just won't necessarily be a Docker container when it's built.
...
**`docker-compose.dev.yml` (bind-mounted source, no rebuild between edits):** added once the "day-to-day dev stays native" advice below started costing real friction. `backend/Dockerfile.dev` / `web/Dockerfile.dev` install dependencies at build time; the compose file bind-mounts the real `backend/`, `personas/`, `skills/`, and `web/` directories over their image counterparts at runtime — Next.js gets true Fast Refresh (verified: edited a page, saw it recompile and serve the change with the container never restarting), and the backend picks up a change with a plain `docker compose restart backend` (also verified — edited a persona's name, restarted, saw the new name over the API — no rebuild). `.venv`/`node_modules`/`.next` each get their own anonymous volume so the bind mount doesn't wipe out installed dependencies, the same trick a Node dev container needs for `node_modules`.

**A real, load-bearing bug found by actually running the built images, not by reading the code:** both `graph/build.py`'s module-level `PERSONAS` dict and `web_adapter/api.py`'s personas-directory default locate `personas/`/`skills/` by walking a fixed number of `parents[N]` up from their own `__file__`, matching the host's `backend/tapestry/...` nesting depth exactly. The original Dockerfiles flattened `tapestry/` straight onto `/app`, one level shallower than the host — inside the built image this resolved to `/personas` (nonexistent), silently returning zero personas from every endpoint and leaving the graph with no persona to route a conversation to at all. Fixed by restructuring both Dockerfiles to mirror the host's exact depth (`/app` plays the repo root, `/app/backend` this package, `/app/personas`/`/app/skills` siblings) rather than touching the already-tested path-resolution code — the code's assumption was reasonable, the image layout just wasn't honoring it. Both `docker-compose.yml` and `docker-compose.dev.yml` now also bind-mount `personas/`/`skills/` from the host (not just bake them into the image), since persona edits made through the running app write back to those YAML files — without the mount, those writes would land on the container's own ephemeral layer and vanish on recreate.

**Day-to-day development**: native only now (`uv`/venv + `pnpm dev` — see the Update note above).

**Ports**: web is `3200`, backend is `8420` (moved off the defaults 3000/8000 to dodge collisions with other services already running locally). The backend binds `TAPESTRY_API_HOST=0.0.0.0` (`backend/.env`), so it's reachable from other interfaces natively, no container publish step needed.

**Reaching Tapestry from another device (e.g. a phone over Tailscale):** the frontend used to bake a single `NEXT_PUBLIC_API_URL=http://localhost:8420` into the browser bundle at build time — correct only from the one host it was built for, and actively wrong from a phone (whose own `localhost` isn't the Mac running the containers). `web/lib/api.ts`'s `resolveApiUrl()` now derives the API's address at runtime instead, from whatever host the page itself was loaded from (`window.location.hostname`) on the backend's port — so the exact same running containers are reachable from `http://localhost:3200` during normal dev *and* from `http://<your-tailscale-hostname>:3200` on a phone on the same tailnet, with no rebuild and no per-device config. `NEXT_PUBLIC_API_URL` still exists as an explicit override for the one case this can't cover on its own — the API genuinely living on a different host/domain than the web app (e.g. separate reverse-proxy subdomains).

The one thing that *does* need explicit configuring is CORS: the backend only accepts browser requests from origins in `TAPESTRY_WEB_ORIGINS` (comma-separated), defaulting to `http://localhost:3200` alone if unset. Reaching the app from a phone means adding that device's origin — e.g. `TAPESTRY_WEB_ORIGINS=http://localhost:3200,http://your-mac-name.your-tailnet.ts.net:3200` in `backend/.env` — since a wildcard origin isn't an option here anyway (the CORS middleware also sets `allow_credentials=True`, which browsers refuse to combine with `Access-Control-Allow-Origin: *`).

Setup, concretely: install Tailscale on the host running the containers and on the phone, join the same tailnet, then browse to `http://<mac's-tailscale-MagicDNS-name>:3200` from the phone. Tailscale's own WireGuard tunnel is already encrypted end-to-end, so plain HTTP over it doesn't need a TLS cert the way exposing this to the public internet would.
