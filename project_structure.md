...
**Day-to-day development**: either native (`uv`/venv + `pnpm dev`, still the lowest-friction option for pure backend-only iteration) or `docker-compose.dev.yml` now that it supports live edits without rebuilds — genuinely equivalent for iteration speed on the web side (Fast Refresh either way) and close to it on the backend (a restart, not a rebuild). `docker-compose.yml` (the production multi-stage build) stays for testing the real deploy artifact, not for iterating against.

**Ports**: web is `3200`, backend is `8420` (moved off the defaults 3000/8000 to dodge collisions with other services already running locally). Both are published on all interfaces by every compose file (`"3200:3200"`/`"8420:8420"`, no host-IP restriction), so nothing about Docker itself limits reachability to `localhost`.
